...
def build_dataset(dataset_dir="dataset", max_per_class=4000):
    root = pathlib.Path(dataset_dir)
    real_dir = root / "real"
    fake_dir = root / "gan_fake"
    real_dir.mkdir(parents=True, exist_ok=True)
    ...
    if n_real < 100:
        raise RuntimeError("Dataset build failed: fewer than 100 real images in dataset/real/")
    if n_gan < 100:
        raise RuntimeError("Dataset build failed: fewer than 100 GAN images in dataset/gan_fake/")

    return n_real, n_gan


def train_layer3(dataset_dir="dataset", epochs=25, save_dir="checkpoints"):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_auc > best_val_auc:
            ...
            print(f"  ✓ SAVED  checkpoints/layer3_best.pth  AUC={val_auc:.4f}  F1={val_f1:.4f}")
        else:
            patience_counter += 1
...
def calibrate_centroid(dataset_dir="dataset", save_dir="checkpoints", n=300):
    device = torch.device("cuda")
    if open_clip is None:
        raise ImportError("open_clip is required. Run: pip install open-clip-torch")
...
if __name__ == "__main__":
    ...
    build_dataset(dataset_dir="dataset", max_per_class=4000)

    print("\n[PHASE 2] Training CLIP head on GPU...")
    best_auc, ckpt_path = train_layer3(dataset_dir="dataset", epochs=25, save_dir="checkpoints")

    print("\n[PHASE 3] Calibrating CLIP centroid...")
    calibrate_centroid(dataset_dir="dataset", save_dir="checkpoints", n=300)
