...
from .base import prepend_sys_path, to_authenticity_from_fraud_probability
...
class CnnInterface:
    def __init__(
        ...
        preferred_fast = self._project_root / "layer1" / "artifacts_fast_85" / "best_model.pth"
        if preferred_fast.exists():
            self._checkpoint = str(preferred_fast)
        else:
