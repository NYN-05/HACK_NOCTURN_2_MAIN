...
class ScoringEngine:
    ...
    def __init__(self, weights: Dict[str, float] | None = None, meta_model: MetaModel | None = None) -> None:
        self.weights = weights or VeriSightConfig.LAYER_WEIGHTS
    ...
    def fuse(
        ...
        available_layers = [key for key in self.weights if availability.get(key, True) and key in normalized_scores]
        ...
        layer_reliabilities = {
            key: max(VeriSightConfig.LAYER_RELIABILITY_FLOOR, min(1.0, float(reliabilities.get(key, 1.0))))
        ...
        if VeriSightConfig.ENABLE_DYNAMIC_RELIABILITY_WEIGHTING:
            dynamic = {key: active_weights[key] * layer_reliabilities[key] for key in available_layers}
            ...
            if dynamic_total > 0:
                ...
                effective_weights = {key: active_weights[key] / active_total if active_total > 0 else 0.0 for key in available_layers}
        else:
            active_total = sum(active_weights.values())
            effective_weights = {key: active_weights[key] / active_total if active_total > 0 else 0.0 for key in available_layers}
...
def create_scoring_engine(
    ...
    return ScoringEngine(weights=weights, meta_model=meta_model)