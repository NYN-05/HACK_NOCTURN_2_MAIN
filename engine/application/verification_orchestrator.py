...
from engine.preprocessing import preprocess_all

from ..configs.weights import EARLY_EXIT_CNN_SCORE_THRESHOLD, EARLY_EXIT_MIN_RELIABILITY, ENABLE_EARLY_EXIT, LAYER_TIMEOUT_MS
...
class VerificationOrchestrator:
    ...
    def load_models(self) -> None:
        for key, loader in (("cnn", self.cnn.load), ("vit", self.vit.load), ("gan", self.gan.load), ("ocr", self.ocr.load)):
            try:
                loader()
    ...
    def _compute_reliability(output: Dict[str, Any], layer_name: str = "cnn") -> float:
        return compute_reliability(output, layer_name)
    ...
    def _skipped_layer_output(layer_name: str) -> Dict[str, Any]:
        return {
            "score": 50.0,
            "raw": {"fallback": "early_exit_skipped", "flags": ["layer_skipped"], "layer": layer_name},
            "available": False,
            "uncertainty": 1.0,
    ...
    def _build_fallback_output(layer_name: str, exc: Exception, elapsed_ms: int) -> Dict[str, Any]:
        return {
            "score": 50.0,
            "raw": {"error": str(exc), "fallback": "uncertain_fallback", "flags": ["layer_failed"], "layer": layer_name},
            "available": False,
            "uncertainty": 1.0,
    ...
    async def _run_layer_async(
        ...
        try:
            output = await asyncio.wait_for(loop.run_in_executor(None, predict, preprocessed, metadata), timeout=timeout_s)
    ...
    async def run(self, image: Any, metadata: Dict[str, Any] | None = None, preprocessed: Dict[str, Any] | None = None) -> Dict[str, Any]:
        started = time.perf_counter()
        metadata = metadata or {}
        preprocessed_bundle = await self._prepare_inputs(image, preprocessed=preprocessed)
        layer_start_event = asyncio.Event()

        layer_tasks = {key: asyncio.create_task(self._run_layer_async(key, preprocessed_bundle, metadata, layer_start_event)) for key in self._layer_scorers}
        ...
        if early_exit_triggered:
            ...
            other_results = await asyncio.gather(layer_tasks["vit"], layer_tasks["gan"], layer_tasks["ocr"], return_exceptions=True)
        ...
        fused = self.scoring.fuse(layer_scores, reliabilities=layer_reliabilities, availability=layer_availability)
    ...
    def run_sync(self, image: Any, metadata: Dict[str, Any] | None = None, preprocessed: Dict[str, Any] | None = None) -> Dict[str, Any]:
