...
DEFAULT_FEATURE_ORDER = ("cnn", "vit", "gan", "ocr")
DEFAULT_MODEL_PATH = Path(__file__).with_name("meta_model.json")
...
class MetaModel:
    ...
    def load(cls, model_path: str | Path | None = None) -> "MetaModel":
        ...
        if not path.exists():
            return cls(DEFAULT_FEATURE_ORDER, (5.4, 2.7, 1.5, 1.2), -5.4, source=str(path), available=False)
    ...
    def predict(
        ...
        feature_vector = {
            name: float(feature_scores.get(name, 50.0)) for name in self.feature_order
        }
        ...
        active_coefficients = {
            name: abs(self.coefficients[index]) for index, name in enumerate(self.feature_order) if name in available_layers
        }
